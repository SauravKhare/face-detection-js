...
const SIZE = 10;
const SCALE = 1.1; // Create new face detector using Chrome API

const faceDetector = new FaceDetector(); // Write a function to populate video from facecam into page
...
function censorFace({
  ...
  ctxFaceCanvas.drawImage( // 5 sourceargs
  webcamVideo, face.x, face.y, face.width, face.height, // 4 location args
  face.x, face.y, SIZE, SIZE);
  const width = face.width * SCALE;
  const height = face.height * SCALE;
  ctxFaceCanvas.drawImage(faceCanvas, face.x, face.y, SIZE, SIZE, face.x, face.y, width, height);
}
